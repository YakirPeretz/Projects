...
clear;
for k=1:1:3
filename = sprintf('primary_image_ext_%d.jpg',k);
I = imread(filename);
Nx = randi([200 720]);
I = imresize(I,[Nx Nx]);
...
figure;
imshow(Iw);
imagname = sprintf('watermarked_image_ext(result)_%d.jpg',k);
imwrite(Iw,imagname);
% Primary Image
outname = sprintf('primary_image_ext_%d.txt',k);
...
outname = sprintf('watermarked_image_ext(result)_%d.txt',k);
fid = fopen(outname, 'wt');
fprintf(fid, '%d\n', N); % decimal writing of image row/cols size
...
outname = sprintf('parameters_random_value_ext_%d.txt',k);
